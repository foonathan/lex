...
namespace lex
{
    ...
    /// CRTP mixin class to generate the `try_match()` for a rule token that matches a single ASCII
    /// character.
    ///
    /// It will match a single character fulfilling `Predicate`. If multiple characters matching the
    /// predicate occur in sequence, multiple tokens will be generated. The `Predicate` can be any
    /// of the [lex::ascii]() functions.
    ///
    /// \notes The derived class must not declare any other functions, just inherit from
    /// it and [lex::rule_token]() or [lex::identifier]().
    template <class Derived, ascii::predicate Predicate>
    struct single_ascii_mixin
    {
        static constexpr auto try_match(const char* str, const char*) noexcept
        {
    ...
    /// CRTP mixin class to generate the `try_match()` for a rule token that matches a single
    /// followed by multiple ASCII characters.
    ///
    /// It will start matching a single character fulfilling `HeadPredicate`, then match all
    /// characters fulfilling `TailPredicate`.
    /// If multiple characters matching the tail predicate occur in sequence, only one token as big
    /// as possible will generated. The predicates can be any of the [lex::ascii]() functions.
    ///
    /// \notes The derived class must not declare any other functions, just inherit from
    /// it and [lex::rule_token]() or [lex::identifier]().
    template <class Derived, ascii::predicate HeadPredicate, ascii::predicate TailPredicate>
    struct head_tail_ascii_mixin
    {
        static constexpr auto try_match(const char* str, const char* end) noexcept
        {
            static_assert(is_rule_token<Derived>::value, "derived class must be a rule token");

            if (!HeadPredicate(*str))
                return Derived::unmatched();
    ...
    /// CRTP mixin class to generate the `try_match()` for a rule token that matches multiple ASCII
    /// characters.
    ///
    /// It will match all characters fulfilling `Predicate`.
    /// If multiple characters matching the predicate occur in sequence, only one token as big as
    /// possible will generated. The `Predicate` can be any of the [lex::ascii]() functions.
    ///
    /// \notes The derived class must not declare any other functions, just inherit from
    /// it and [lex::rule_token]() or [lex::identifier]().
    template <class Derived, ascii::predicate Predicate>

...
namespace lex
{
    template <class Derived, class Grammar, class ProductionOrToken, class Separator>
    class list_production : public detail::base_production
    {
        static_assert(is_production<ProductionOrToken>::value || is_token<ProductionOrToken>::value,
                      "list element must be a production or token");
        static_assert(is_token<Separator>::value, "list separator must be a token");

        template <class Func>
        static constexpr auto parse_elem(std::true_type /* is_production */,
        ...
        template <class Func>
        static constexpr auto parse_elem(std::false_type /* is_production */,
                                         tokenizer<typename Grammar::token_spec>& tokenizer,
                                         Func&&                                   f)
        {
            auto token = tokenizer.peek();
            using result_type
                = parse_result<decltype(detail::parse_token<ProductionOrToken>(token))>;
            if (token.is(ProductionOrToken{}))
            {
                tokenizer.bump();
                return result_type::success(detail::parse_token<ProductionOrToken>(token));
            }
            else
            {
                auto error
                    = unexpected_token<Grammar, Derived, ProductionOrToken>(Derived{},
                                                                            ProductionOrToken{});
                                         ...
                                         Func&&                                   f)
        {
            ...
            auto elem = parse_elem(is_production<ProductionOrToken>{}, tokenizer, f);
            ...
            result
                = lex::detail::apply_parse_result(f, Derived{}, result.template forward<Derived>(),
                                                  elem.template forward<ProductionOrToken>());
            ...
            while (tokenizer.peek().is(Separator{}))
            {
                ...
                elem = parse_elem(is_production<ProductionOrToken>{}, tokenizer, f);
                if (elem.is_unmatched())
                    return decltype(result)::unmatched();
        ...
        static constexpr auto parse_impl(std::false_type /* allow_empty */,
            ...
            -> decltype(lex::detail::apply_parse_result(
                ...
                parse_elem(is_production<ProductionOrToken>{}, tokenizer, f)
                    .template forward<ProductionOrToken>()))
        {
            auto elem = parse_elem(is_production<ProductionOrToken>{}, tokenizer, f);
            if (elem.is_unmatched())
                return {};
    ...
    template <class Derived, class Grammar, class Open, class ProductionOrToken, class Separator,
              class Close>
    ...
    public:
        ...
        template <class Func>
        static constexpr auto parse(tokenizer<typename Grammar::token_spec>& tokenizer, Func&& f)
            -> decltype(list_production<Derived, Grammar, ProductionOrToken, Separator>::parse(
                tokenizer, f))
        {
            ...
            auto result
                = list_production<Derived, Grammar, ProductionOrToken, Separator>::parse(tokenizer,
                                                                                         f);
